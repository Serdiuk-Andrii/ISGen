...
class panel:
        ...
        for run in range(runs):
            ...
            for clausenum in self.ind_clause_dict[bestind]:
                ...
                for ind in set(self.clause_list[clausenum]):
                    try:
                        if self.count_self is True:
                            self.update_selected_coverage[self.selected_probands_dict[ind]] -= self.clause_status[
                                                                                                   clausenum] * \
                                                                                               (len(self.clause_list[
                                                                                                        clausenum]))
                        elif self.count_self is False:
                            self.update_selected_coverage[self.selected_probands_dict[ind]] -= self.clause_status[
                                                                                                   clausenum] * \
    ...
    def randompanel(self, numinds):
        ...
        rand_indices = random.sample(range(len(self.selected_probands)), numinds)
    ...
    def createfile(self, outfile):
        ...
        if os.path.isfile(outfile) is True:
            ...
            print(f"Warning: output file already exists. Outputting to new file {outfile}")
    ...
    def run(self, panel_out_path, panelsizes, random_panels):
        ...
        for numinds in panelsizes:
            print(f"Building panels of {numinds} individuals...")
