...
def get_numbhidden_transmissions(Pedigree, tree):
    founders = []
    tree = set(tree)
    for ind in tree:
...
def main(args):
    ...
    with tables.open_file(climb_lik_file, 'r') as f:
        ...
        for i, tree in enumerate(f.root.trees):
            num_hidden_transmissions = get_numbhidden_transmissions(P, tree)
            scale = 1. / (len(tree) - num_hidden_transmissions)
            haplotype_liks[i] = scipy.stats.erlang(2, scale=scale).pdf(hap_length)
            if i % 10000 == 0:
                print("Calculating haplotype likelihoods for tree number", i, "of", len(haplotype_liks))
