...
def main(args):
    ...
    max_kinship_count = np.max(
        np.histogram2d(np.log10(np.array(true_kinship_vals)), np.log10(np.array(est_kinship_vals)), bins=bins)[0])
    max_ISGen_count = np.max(
        np.histogram2d(np.log10(np.array(true_ISGen_vals)), np.log10(np.array(est_ISGen_vals)), bins=bins)[0])
